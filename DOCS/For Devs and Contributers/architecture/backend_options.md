...
Firebase plan (Spark/free) — confirmed in `issues_fixed.md` §24 and blocking two of
...
- `issues_open.md` §33.6 (half) — `storage.rules` could mirror a ride's
...
- **All of `firestore.rules`, as Postgres Row-Level Security policies.**
  ...
  `issues_fixed.md` §24.7), audience-tiered visibility (`rideVisibleTo`,
...
(`issues_fixed.md` §3, §10, §24, §33) — it's the right call if there's an actual
