    ...
    return Scaffold(
      ...
      body: rideAsync.when(
        ...
        data: (ride) {
          if (ride == null) {
            return const Center(
                child: Text('Ride not found', style: TextStyle(color: AppColors.textSecondary)));
          }

          final startCenter = _polyline.isNotEmpty ? _polyline.first : const LatLng(23.8103, 90.4125);
              ...
              children: [
                ...
                if (ride.mapSnapshotPath != null)
                  ...
                  ClipRRect(
                    ...
                    child: SizedBox(
                      height: 200,
                      child: GoogleMap(
                        initialCameraPosition: CameraPosition(target: startCenter, zoom: 14),
                        polylines: {
                          if (_polyline.length > 1)
                            Polyline(
                              polylineId: const PolylineId('summary'),
                              points: _polyline,
                ...
                GridView.count(
                  ...
                  children: [
                    ...
                    StatCard(
                      label: 'Duration',
                      value: SpeedFormatter.durationFromSeconds(ride.durationSeconds ?? 0),
                    ...
                    StatCard(
                      ...
                      valueColor: ride.maxSpeedKmh > 100 ? AppColors.warning : null,
                    ),
                  ],
                ),
...
class _EventCard extends StatelessWidget {
  ...
  final String label;
  final int count;
  const _EventCard({required this.icon, required this.color, required this.label, required this.count});
  ...
  Widget build(BuildContext context) {
    ...
    final color = score >= 80
        ...
        : score >= 60
            ? AppColors.warning
            : AppColors.danger;
    final label = score >= 80 ? 'Smooth Rider' : score >= 60 ? 'Average' : 'Aggressive';
