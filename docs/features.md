...
- **Settings screen** (`/settings`) — profile summary, **Appearance** (skin picker, see below), **Emergency Contacts** (add/delete; the description under this section is honest that delivery isn't live yet — see below), **Sign Out**.
  ...
  - **Appearance → Skin** (changed 2026-08-11; grew to ten skins 2026-08-17) — the two-way *Carbon Mono* / *Editorial* switch is now a **dropdown of ten skins**. Each row carries the skin's name, a one-line blurb, and a **swatch painted in that skin's own palette** (its background plus its primary and secondary accents) — with ten names and no preview the control would be a list of words. See `app/lib/features/profile/presentation/widgets/skin_dropdown.dart`.
    ...
    | **Cute Analyst** *(added 2026-08-17)* | navy console | cyan / coral | **rounded** |

    Cute Analyst is Analyst Blue's exact palette (`AppColorPalette.cuteAnalyst` is a straight `const` alias, not a re-derived copy) with the shape swapped from `boxy` to `rounded` — the first skin to deliberately reuse another skin's colors, added to demonstrate that color and shape are genuinely independent axes rather than always moving together.
